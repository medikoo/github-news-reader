...
let isFunction = require("es5-ext/function/is-function")
...
logAPIError = memoize((message, article, err) => {
...
parse = function (re, map, normalize, article) {
	let match = article.link.match(re), scope = this, name, names = [];
	if (!match) {
...
parseUser = function (name, normalize, article) {
	let author = article.author, scope;
...
titleFromBlockquote = function (article) {
	const match = article.description
...
process = {
	...
	pullrequestreviewcommentevent: partial.call(parse, new RegExp("^https:" +
		...
		"\\/(\\d+)#(?:discussion_)?r(\\d+)$"),
		["Repo", 1, "Issues & Pull Requests", 2], (article, orgmatch) => {
			let data;
			fixLinks(article);
			if (contains.call(article.description, "…</p>")) {
				data = orgmatch[1].split("/");
